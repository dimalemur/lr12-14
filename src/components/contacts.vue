  ...
  <div id="app">
    <div class="container">
      ...
      <div class="mt-2 mb-5">
        ...
        <div class="row">
          <input
            ...
            name="sort"
            value="surname"
            v-on:click="contactsSort('surname')"
          />
          <input
            class="col"
            type="radio"
      ...
      <div class="row mt-2" v-for="contact in contacts" v-bind:key="contact.id">
        ...
        <div class="col-1">
          <span v-if="contact.fav">♥</span>
        </div>
        <div class="col-2">
          <button class="btn btn-warning" v-on:click="fillForm(contact)">Изменить</button>
        </div>
        <div class="col-1">
          <button class="btn btn-danger" v-on:click="deleteContact(contact.id)">X</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async addContact() {
      ...
      try {
        await this.$http.post("/contacts", contact);
    ...
    async updateContactsList() {
      try {
        let res = await this.$http.get("/contacts");
    ...
    fillForm(contact) {
      ...
      this.inIdCont = this.contacts.findIndex(
